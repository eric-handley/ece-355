...
volatile uint32_t firstEdge = 0; //Holds the timer value at the first edge
volatile uint32_t secondEdge = 0; //Holds the timer at the second edge
	...
	if ((EXTI->PR & EXTI_PR_PR2) != 0)
	{
		...
		*/
		...
		else
		{
			...
			uint32_t period = secondEdge; //In clock cycles
			uint32_t frequency = SystemCoreClock / secondEdge + 1;

			trace_printf("Period: is %u cycles\n", period);
			trace_printf("Frequency: is %u Hz \n", frequency);
